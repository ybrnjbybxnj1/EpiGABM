...
try:
    ...
    _HAS_HMMLEARN = False
    logger.debug("hmmlearn not available; GaussianMixture fallback "
                 "(this is NOT a real HMM)")
...
class HMMDetector(BaseDetector):
    def __init__(
        ...
        self.require_hmm = require_hmm

        if not _HAS_HMMLEARN and require_hmm:
            raise ImportError(
                "HMMDetector(require_hmm=True) was constructed but the "
                "hmmlearn package is not importable in this environment. "
                "Install hmmlearn  -  for python 3.13/3.14 use the analyses "
                "python-3.12 venv: "
                "`.venv-tf/Scripts/python.exe -m pip install hmmlearn`  -  "
                "or pass require_hmm=False to opt into the GMM fallback "
                "(which is NOT an HMM)."
            )
        ...
        if _HAS_HMMLEARN:
            ...
            self._actual_model_type = "gmm_fallback"
    ...
    @property
    def actual_model_type(self) -> str:
        """'hmm' = real Gaussian HMM with Viterbi decoding;
        'gmm_fallback' = sklearn GaussianMixture surrogate when hmmlearn is
        unavailable. exp03 records this in its JSON output so any
        published claim about HMM performance can be cross-checked against
