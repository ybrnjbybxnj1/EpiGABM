...
function showBuilding(b) {{
  ...
  const expo=b.exposed||0;
  const parts=[`${{b.total}} people`];
  if(b.infected>0) parts.push(`${{b.infected}} infectious`);
  if(expo>0) parts.push(`${{expo}} exposed`);
...
function setDay(d) {{
  ...
  // refresh agent diary if one is selected, otherwise refresh building panel
  if(selAgent) selectAgent(selAgent, selArch);
  else if(lastBuilding) {{
    const fresh=(DD[cur]||[]).find(b=>b.hh_id===lastBuilding.hh_id);
    if(fresh) showBuilding(fresh);
  }}
}}
