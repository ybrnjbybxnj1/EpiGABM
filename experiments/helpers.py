...
def make_small_population(
    n: int = 100,
    seed: int = 42,
    population_dir: str = "data/population",
    fallback_synthetic: bool = True,
) -> tuple[pd.DataFrame, pd.DataFrame]:
    """Load a household-coherent subsample of the JoCS Saint-Petersburg
    synthetic population (people.txt + households.txt). Subsamples whole
    households so children stay with their parents and household
    transmission stays well-defined.

    The original (pre-2026-05) implementation of this function generated
    a synthetic population on the fly via numpy RNG, which silently
    bypassed the JoCS files supervisor explicitly required to be the
    population source. This version delegates to
    src.utils.data_loader.load_data and uses the rng-based generation
    only as a last-resort fallback when the JoCS files are absent
    (with an explicit warning so the bypass cannot happen unnoticed).
    """
    pop_dir = Path(population_dir)
    people_path = pop_dir / "people.txt"
    households_path = pop_dir / "households.txt"
    if people_path.exists() and households_path.exists():
        from src.utils.data_loader import load_data, preprocess_data
        data, households, _ = load_data(
            population_dir=pop_dir, n_agents=n, seed=seed,
        )
        data, households = preprocess_data(data, households)
        used_hh = set(int(h) for h in data["sp_hh_id"].unique())
        households = households[households["sp_id"].isin(used_hh)].copy()
        return data, households

    if not fallback_synthetic:
        raise FileNotFoundError(
            f"JoCS files not found under {pop_dir}; "
            f"populate {people_path.name} and {households_path.name} "
            f"or pass fallback_synthetic=True to enable rng-based generation."
        )
    logger.warning(
        "JoCS population files not found at {}; falling back to synthetic "
        "rng-generated population. This is NOT what the supervisor's brief "
        "specified  -  install data/population/{{people,households}}.txt "
        "to use the canonical JoCS subsample.",
        pop_dir,
    )
