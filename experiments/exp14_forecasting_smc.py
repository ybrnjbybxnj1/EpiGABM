...
def forecast_hybrid_quadratic_envelope(
    gabm_result, T_obs, H, N, sigma, gamma_bio,
    seed=0, _legacy_n_particles_unused=300,
):
...
def run_experiment(config_path="config/default.yaml", seed=42,
    ...
    per_seed_metrics = {m: [] for m in
                        ["persistence", "exp", "sir", "hybrid_quad"]}
    truths = []
    ...
    for idx, (traj, gabm_result) in enumerate(zip(trajectories, results_list)):
        ...
        f_quad = forecast_hybrid_quadratic_envelope(
            gabm_result, T, H, n_agents, sigma, gamma_bio, seed=idx,
        )

        per_seed_forecasts.append({
            "persistence": f_persist, "exp": f_exp, "sir": f_sir,
            "hybrid_quad": f_quad, "truth": I_true,
        })
        ...
        per_seed_metrics["hybrid_quad"].append(_metrics(f_quad["median"], I_true))
    ...
    for fc in per_seed_forecasts:
        ...
        for h in METRIC_HORIZONS:
            ...
            horizon_err["hybrid_quad"][h].append(
                abs(fc["hybrid_quad"]["median"][h - 1] - truth[h - 1]))
    ...
    if per_seed_forecasts:
        ...
        smc_mean = np.concatenate([[last_obs], fc["hybrid_quad"]["mean"]])
        smc_map = np.concatenate([[last_obs], fc["hybrid_quad"]["map"]])
        smc_lo = np.concatenate([[last_obs], fc["hybrid_quad"]["lower"]])
        smc_hi = np.concatenate([[last_obs], fc["hybrid_quad"]["upper"]])
    ...
    methods_viz = ["persistence", "sir", "hybrid_quad"]
    colors = {"persistence": "#888", "sir": "#2ca02c", "hybrid_quad": "#9467bd"}
    bar_w = 0.25
    ...
    if len(truths) >= 2:
        ...
        all_medians = np.array([f["hybrid_quad"]["median"] for f in per_seed_forecasts])
        all_lo = np.array([f["hybrid_quad"]["lower"] for f in per_seed_forecasts])
        all_hi = np.array([f["hybrid_quad"]["upper"] for f in per_seed_forecasts])
