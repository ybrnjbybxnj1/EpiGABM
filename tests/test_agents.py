...
def test_parse_decision_strips_think_block():
    raw = (
        "<think>Let me reason about this. I am a healthcare worker, so I "
        "should isolate. The flu is dangerous.</think>\n"
...
def test_parse_decision_strips_reasoning_tag():
    raw = (
        "<reasoning>I think I should isolate.</reasoning>"
        + json.dumps(_VALID_DECISION_PAYLOAD)
    )
    decision = parse_decision(raw)
    assert decision.isolate is True
...
def test_parse_decision_extracts_json_from_prose():
    raw = (
        "Sure, here is my response based on my situation as a healthcare "
        "worker:\n\n" + json.dumps(_VALID_DECISION_PAYLOAD) +
...
def test_parse_decision_think_plus_prose_plus_fence():
    raw = (
...
def test_parse_decision_no_json_anywhere_raises():
    raw = "<think>only thinking, no answer at all</think> sorry, I cannot answer"
    with pytest.raises(ValueError, match="invalid json"):
        parse_decision(raw)


def test_parse_decision_bpe_artifact_normalisation():
    fenced_json = "```json\n" + json.dumps(_VALID_DECISION_PAYLOAD) + "\n```"
    mangled = (fenced_json
               .replace(" ", "Ġ")
               .replace("\n", "Ċ"))
    decision = parse_decision(mangled)
    assert decision.isolate is True
    assert decision.reduce_contacts is True
    assert decision.reasoning == "test"
